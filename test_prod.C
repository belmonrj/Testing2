...
void flatten(int runnumber, int passnumber)
{
  ...
  string pipe_out = (string) gSystem->GetFromPipe(Form("ls input_test_prod/tree_0000%d_*.root | grep -c r", runnumber));
  int nfiles = 0;
  ...
  for ( int ifile = 0; ifile < nfiles; ++ifile )
    {
      ...
      sprintf(filename, "input_test_prod/tree_0000%d_%04d.root", runnumber, ifile);
