...
void plot_v2eta_pAu()
{
  ...
  TFile* infile = TFile::Open("pau200_all.root");
  //TFile* infile = TFile::Open("pau200_roncode.root"); // Ron code run by Ron
  //TFile* infile = TFile::Open("pau200_darrencoderunbyron.root"); // Darren code run by Ron
  ...
  c1->Print("Figures/v2eta_zbins_pau200.png");
  c1->Print("Figures/v2eta_zbins_pau200.pdf");
  ...
  c1->Print("Figures/v2eta_zcheck_pau200.png");
  c1->Print("Figures/v2eta_zcheck_pau200.pdf");


  // ----------------------------------------------------------------
  // --- now make the plot with the event plane resolution correction

  double epreso = 0.062; // from AN1234
  ...
  c1->Print("Figures/v2eta_corr_pau200.png");
  c1->Print("Figures/v2eta_corr_pau200.pdf");
  ...
  c1->Print("Figures/v2eta_clean_pau200.png");
  c1->Print("Figures/v2eta_clean_pau200.pdf");

  // -------------------------------------------------------------
  // --- now make the final plot with the eta dependent correction

  TFile* corrfile = TFile::Open("ampt_vneta_correction.root");
  TH1D* v2eta_corr = (TH1D*)corrfile->Get("v2eta_correction_dAu200"); // need one for pAu200, but this should be pretty close
  ...
  c1->Print("Figures/v2eta_final_pau200.png");
  c1->Print("Figures/v2eta_final_pau200.pdf");

  TFile* outfile = TFile::Open("pau200_v2eta.root","recreate");
  outfile->cd();
